...
def buying(name):
    ...
    print("Total Amount Required to buy: "+str((int(amount)*int(book_to_buy[0].price))))
    if (int(buyer_account[0].balance)> (int(amount)*int(book_to_buy[0].price))):
        ...
        print("Transaction Request Sent to Bank")
...
def bank():
    ...
    db.session.commit()
    ecommerce=Bankuser.query.get_or_404(101)
    ...
    db.session.commit()
    print("Bank Has Settled the Transaction from buyer to ecommerce")
    ...
    db.session.commit()
    print("The Transaction id is:"+str(x))
...
def seller():
    ...
    ecommerce.balance=old_balance_ecommerce-(amount)*0.8
    sellerid=seller[0].id
    seller1=Bankuser.query.get_or_404(sellerid)
    old_balance_seller=seller1.balance
    seller1.balance=old_balance_seller+(amount)*0.8
    db.session.commit()
    print("Bank Has Selleed the Transaction from ecommerce to seller")
    print("Transaction Id is"+str( transaction_id))
...
def verify():
    ...
    tran=Transaction.query.get_or_404(transaction)
    # status='Not Verified'
    # amount=0
    if  int(tran.id)==int(transaction):
    ...
    return render_template("verifying.html",status=status,amount=amount)
